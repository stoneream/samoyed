...
object ArtistAlbumFetchSchedule extends SQLSyntaxSupport[ArtistAlbumFetchSchedule] {
  ...
  override val tableName = "artist_album_fetch_schedule"
  ...
  def find(id: Int)(implicit session: DBSession): Option[ArtistAlbumFetchSchedule] = {
    withSQL {
  ...
  def create(
    ...
    updatedAt: OffsetDateTime,
    deletedAt: Option[OffsetDateTime] = None,
    lockVersion: Int)(implicit session: DBSession): ArtistAlbumFetchSchedule = {
    val generatedKey = withSQL {
      insert.into(ArtistAlbumFetchSchedule).namedValues(
  ...
  def batchInsert(entities: collection.Seq[ArtistAlbumFetchSchedule])(implicit session: DBSession): List[Int] = {
    ...
    SQL("""insert into artist_album_fetch_schedule(
      artist_id,
      scheduled_at,
      started_at,
      finished_at,
  ...
  def save(entity: ArtistAlbumFetchSchedule)(implicit session: DBSession): ArtistAlbumFetchSchedule = {
    withSQL {
      update(ArtistAlbumFetchSchedule).set(
        column.id -> entity.id,
