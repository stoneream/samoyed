...
object ArtistAlbumDetailFetchSchedule extends SQLSyntaxSupport[ArtistAlbumDetailFetchSchedule] {
  ...
  override val tableName = "artist_album_detail_fetch_schedule"
  ...
  def find(id: Int)(implicit session: DBSession): Option[ArtistAlbumDetailFetchSchedule] = {
    withSQL {
  ...
  def create(
    ...
    updatedAt: OffsetDateTime,
    deletedAt: Option[OffsetDateTime] = None,
    lockVersion: Int)(implicit session: DBSession): ArtistAlbumDetailFetchSchedule = {
    val generatedKey = withSQL {
      insert.into(ArtistAlbumDetailFetchSchedule).namedValues(
  ...
  def batchInsert(entities: collection.Seq[ArtistAlbumDetailFetchSchedule])(implicit session: DBSession): List[Int] = {
    ...
    SQL("""insert into artist_album_detail_fetch_schedule(
      artist_album_id,
      scheduled_at,
      started_at,
      finished_at,
  ...
  def save(entity: ArtistAlbumDetailFetchSchedule)(implicit session: DBSession): ArtistAlbumDetailFetchSchedule = {
    withSQL {
      update(ArtistAlbumDetailFetchSchedule).set(
        column.id -> entity.id,
