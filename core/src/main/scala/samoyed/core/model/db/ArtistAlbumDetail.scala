...
object ArtistAlbumDetail extends SQLSyntaxSupport[ArtistAlbumDetail] {
  ...
  override val columns =
    Seq("id", "artist_album_id", "album_name", "release_date", "release_date_type", "label", "created_at", "updated_at", "deleted_at", "lock_version")
  ...
  def find(id: Long)(implicit session: DBSession): Option[ArtistAlbumDetail] = {
    withSQL {
  ...
  def create(
      ...
      deletedAt: Option[OffsetDateTime] = None,
      lockVersion: Int
  )(implicit session: DBSession): ArtistAlbumDetail = {
    val generatedKey = withSQL {
      insert
        .into(ArtistAlbumDetail)
        .namedValues(
  ...
  def batchInsert(entities: collection.Seq[ArtistAlbumDetail])(implicit session: DBSession): List[Int] = {
    ...
    SQL("""insert into artist_album_details(
      artist_album_id,
      album_name,
      release_date,
      release_date_type,
